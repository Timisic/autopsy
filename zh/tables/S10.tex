...
\bottomrule
\end{tabular}
\tabnote{NA 为消极情绪，PA 为积极情绪，LS 为生活满意度。$n=104$。差值为候选模型减去比较模型。逐点区间来自 5,000 次配对 bootstrap，检验采用 10,000 次 MSE 预测交换，Holm 覆盖六行。原表后三行的旧字段 shared\_vs\_independent 实际表示候选模型减去原 RF；本表按 model\_a 与 model\_b 字段命名，未改变方向或数值。这些检验不恢复验证性地位。}
\endgroup
\end{table}
